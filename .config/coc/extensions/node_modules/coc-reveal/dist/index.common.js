...
function parseMarkdown(content) {
  ...
  if (lines[index].trimRight() === '---') {
    ...
    for (index += 1; index < lines.length; index += 1) {
      ...
      if (line === '---') {
        ...
        try {
          Object.assign(frontMatter, jsYaml.load(raw.join('\n')));
        } catch {// noop
        }
  ...
  for (; index < lines.length; index += 1) {
    ...
    (_section = section) == null ? void 0 : _section.lines.push(line);
  }
...
function buildSlides(content) {
  ...
  for (const section of sections) {
    ...
    data.push('<section data-markdown>', '<script type="text/template">', section.content, '</script>', '</section>');
  }

  data.unshift('<section>');
  data.push('</section>');
  const jsdelivrPrefix = 'https://cdn.jsdelivr.net/combine/';
  const revealPrefix = `npm/reveal.js@${"4.1.3"}/`;
  const revealCss = ['dist/reveal.css', `dist/theme/${frontMatter.theme}.css`, `plugin/highlight/${frontMatter.highlightTheme}.css`].map(s => revealPrefix + s).join(',');
  frontMatter.css.unshift(jsdelivrPrefix + revealCss);
  const revealJs = ['dist/reveal.min.js', 'plugin/markdown/markdown.js', 'plugin/highlight/highlight.js', 'plugin/math/math.js', 'plugin/notes/notes.js', 'plugin/search/search.js', 'plugin/zoom/zoom.js'].map(s => revealPrefix + s).join(',');
...
async function createReveal(options) {
  ...
  if (options.open) open__default(options.output);
}

async function revealIt(content, options) {
  const {
    nvim
  } = coc_nvim.workspace;
...
function activate(context) {
  context.subscriptions.push(coc_nvim.workspace.registerKeymap(['n'], 'reveal-it', async () => {
    await revealIt(await getFullText());
  }, {
    sync: false
  }));
  context.subscriptions.push(coc_nvim.workspace.registerKeymap(['v'], 'reveal-it-v', async () => {
    await revealIt(await getSelectedText());
  }, {
    sync: false
  }));
  context.subscriptions.push(coc_nvim.commands.registerCommand('reveal.it', async (...args) => {
    const positional = [];
    const options = {};

    for (const arg of args) {
      if (['-w', '--watch'].includes(arg)) options.watch = true;else if (['--no-open'].includes(arg)) options.open = false;else if (!arg.startsWith('-')) positional.push(arg);
    }
